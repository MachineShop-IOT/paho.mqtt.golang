...
func (store *MemoryStore) WithMilieu(milieu *Milieu) *MemoryStore {
	store.milieu = milieu
	return store
}
...
func (store *MemoryStore) Open() {
	...
	defer store.Unlock()
	store.opened = true
	DEBUGD.Dumpln(*store.milieu, STR, "memorystore initialized")
}
...
func (store *MemoryStore) Put(key string, message packets.ControlPacket) {
	...
	defer store.Unlock()
	if !store.opened {
		ERRORD.Dumpln(*store.milieu, STR, "Trying to use memory store, but not open")
		return
	}
...
func (store *MemoryStore) Get(key string) packets.ControlPacket {
	...
	if m == nil {
		CRITICALD.Dumpln(*store.milieu, STR, "memorystore get: message", mid, "not found")
	} else {
		DEBUGD.Dumpln(*store.milieu, STR, "memorystore get: message", mid, "found")
	}
	return m
}
...
func (store *MemoryStore) Del(key string) {
	...
	m := store.messages[key]
	if m == nil {
		WARND.Dumpln(*store.milieu, STR, "memorystore del: message", mid, "not found")
	} else {
		delete(store.messages, key)
		DEBUGD.Dumpln(*store.milieu, STR, "memorystore del: message", mid, "was deleted")
	}
}
...
func (store *MemoryStore) Close() {
	...
	defer store.Unlock()
	if !store.opened {
		ERRORD.Dumpln(*store.milieu, STR, "Trying to close memory store, but not open")
		return
	}
	store.opened = false
	DEBUGD.Dumpln(*store.milieu, STR, "memorystore closed")
}
...
func (store *MemoryStore) Reset() {
	...
	defer store.Unlock()
	if !store.opened {
		ERRORD.Dumpln(*store.milieu, STR, "Trying to reset memory store, but not open")
	}
	store.messages = make(map[string]packets.ControlPacket)
	WARND.Dumpln(*store.milieu, STR, "memorystore wiped")
}
