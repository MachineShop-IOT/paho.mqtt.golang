...
func persistOutbound(milieu *Milieu, s Store, m packets.ControlPacket) {
	...
	case 1:
		...
		default:
			ERRORD.Dumpln(*milieu, STR, "Asked to persist an invalid message type")
		}
	case 2:
		switch m.(type) {
...
func persistInbound(milieu *Milieu, s Store, m packets.ControlPacket) {
	switch m.Details().Qos {
	case 0:
		switch m.(type) {
		...
		default:
			ERRORD.Dumpln(*milieu, STR, "Asked to persist an invalid messages type")
		}
	case 1:
		switch m.(type) {
